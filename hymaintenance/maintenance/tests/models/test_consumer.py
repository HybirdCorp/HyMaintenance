...
class MaintenanceConsumerTestCase(TestCase):
    def test_i_can_create_a_maintenance_consumer(self):
        ...
        MaintenanceConsumer.objects.create(name="Isaac Kleiner", company=company)
...
class MaintenanceConsumerManagerTestCase(TestCase):
    def test_get_used_consumers(self):
        company = CompanyFactory()
        MaintenanceConsumer.objects.create(name="Isaac Kleiner", company=company, is_used=True)
        MaintenanceConsumer.objects.create(name="Isaac Kleiner", company=company, is_used=False)
        self.assertEqual(1, MaintenanceConsumer.objects.get_used_consumers().count())
        self.assertEqual("Isaac Kleiner", MaintenanceConsumer.objects.get_used_consumers().first().name)
