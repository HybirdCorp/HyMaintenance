...
class CreateOperatorTestCase(TestCase):
    ...
    def test_get_create_operator_form(self):
        response = self.client.get(
            reverse("high_ui:project-create_operator", kwargs={"company_name": self.company.slug_name}), follow=True
        )
    ...
    def test_create_operator_with_form(self):
        ...
        response = self.client.post(
            reverse("high_ui:project-create_operator", kwargs={"company_name": self.company.slug_name}),
...
class UpdateOperatorWithCompanyTestCase(TestCase):
    ...
    def test_get_create_operator_form(self):
        response = self.client.get(
            reverse(
                ...
                kwargs={"company_name": self.company.slug_name, "pk": self.operator.pk},
            ),
            follow=True,
        )
    ...
    def test_create_operator_with_form(self):
        ...
        self.assertRedirects(
            response, reverse("high_ui:project-update_operators", kwargs={"company_name": self.company.slug_name})
        )
        self.assertEqual(
            1,
            MaintenanceUser.objects.filter(
...
class UpdateOperatorTestCase(TestCase):
    ...
    def test_get_create_operator_form(self):
        response = self.client.get(reverse("high_ui:update_operator", kwargs={"pk": self.operator.pk}), follow=True)
    ...
    def test_create_operator_with_form(self):
        ...
        response = self.client.post(
            reverse("high_ui:update_operator", kwargs={"pk": self.operator.pk}),
            {"first_name": first_name, "last_name": last_name, "email": email, "password": "letmein"},
            follow=True,
        )
...
class UpdateOperatorUsersTestCase(TestCase):
    @classmethod
    def setUpTestData(cls):
        OperatorUserFactory(email="gordon.freeman@blackmesa.com", password="azerty", id=1)

    def setUp(self):
        self.client.login(username="gordon.freeman@blackmesa.com", password="azerty")

    def test_get_update_operators_form(self):
        response = self.client.get(reverse("high_ui:update_operators"), follow=True)
    ...
    def test_post_archive_operators_form(self):
        ...
        response = self.client.post(reverse("high_ui:archive_operators"), {"active_operators": op_id}, follow=True)

        self.assertEqual(response.status_code, 200)
        self.assertRedirects(response, reverse("high_ui:dashboard"))
        self.assertEqual(1, MaintenanceUser.objects.filter(email=op_email, is_active=False).count())
    ...
    def test_post_unarchive_operators_form(self):
        ...
        response = self.client.post(reverse("high_ui:unarchive_operators"), {"inactive_operators": op_id}, follow=True)

        self.assertEqual(response.status_code, 200)
        self.assertRedirects(response, reverse("high_ui:dashboard"))
        self.assertEqual(1, MaintenanceUser.objects.filter(email=op_email, is_active=True).count())
...
class UpdateOperatorUsersWithCompanyTestCase(TestCase):
    ...
    def test_get_update_company_operators_form(self):
        response = self.client.get(
            reverse("high_ui:project-update_operators", kwargs={"company_name": self.company.slug_name}), follow=True
        )
    ...
    def test_post_update_company_operators_form(self):
        ...
        response = self.client.post(
            reverse("high_ui:project-update_operators", kwargs={"company_name": self.company.slug_name}),
            {"users": operator.id},
            follow=True,
        )
